...
class TestDeployment():
    def test_config_reload(self, duthost, unctrl_links,
                      profile_name, default_priority, cipher_suite,
                      primary_cak, primary_ckn, policy, send_sci):
        # Only pick one uncontrolled link for mismatch macsec configuration test
        assert unctrl_links
        ...
        # Save config with MACsec configure
        nbr["host"].shell("sonic-cfggen -d --print-data > /etc/sonic/config_db.json")
        config_reload(nbr["host"])
        assert wait_until(300, 6, 12, _test_appl_db)
