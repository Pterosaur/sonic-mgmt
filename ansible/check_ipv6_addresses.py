...
def get_lab_location_from_dut(dut_name):
    """Determine lab location from DUT name."""
    if dut_name.startswith('str'):
        # Extract the location from names like 'str4-sn5640-2' or 'str2-msn2700-spy-1'
        match = re.match(r'str(\d*)-', dut_name)
        if match:
            return 'str'
    elif dut_name.startswith('bjw'):
        return 'bjw'
    elif dut_name.startswith('svc'):
        return 'svc'
    elif dut_name.startswith('svcstr'):
        return 'svc'
    elif dut_name.startswith('strtk'):
        # For testbeds with strtk DUTs, they typically use str location
        return 'str'
